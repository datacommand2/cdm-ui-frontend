...
const ProtectionGroupInstances = ({
    ...
    initTenant,
    initInstances,
}: ProtectionGroupInstancesProps) => {
    console.log(initTenant);
    const theme = useTheme();
    const { t } = useTranslation();

    const [selectedTenantID, setSeletedTenantID] = useState('all');

    // 보호대상과 비보호 대상인스턴스의 아이디 값만 저장
    const [unProtectedInstances, setUnprotectedInstances] = useState<any[]>([]);
    ...
    const { data: clusterTenants } = useQuery(
        clusterTenantKeys.list({ clusterID: getValues('protection_cluster.id') }),
        () => _getClusterTenant(getValues('protection_cluster.id')),
        {
            select: ([data, , status]) => {
            ...
            enabled: getValues('protection_cluster.id') !== 0,
        },
    );

    // 클러스터 비보호 인스턴스 목록을 조회하는 함수
    const { data: unprotectedInstances, isFetching: instanceLoading } = useQuery(
        unprotectedInstanceKeys.list({ clusterID: getValues('protection_cluster.id'), tenantID: selectedTenantID }),
        () => _getUnprotectedInstance(getValues('protection_cluster.id'), selectedTenantID),
        {
            select: ([data, , status]) => {
                if (status === 200 || status === 201) {
            ...
            enabled: !!protectionGroupDetail && !!selectedTenantID,
    ...
    useEffect(() => {
        if (unprotectedInstances) {
            // 바꾼 프로젝트가 처음 프로젝트거나 전체 프로젝트면
            // 비보호 인스턴스 목록은
            // 비보호 인스턴스 + (init 인스턴스 목록 - 현재 보호 인스턴스)
            const protectedInstances = getValues('instances');

            if (selectedTenantID === 'all') {
                // init 인스턴스 - 현재 보호 인스턴스
                let instnaces = unprotectedInstances.filter(
                    (instance: any) => !protectedInstances.some((ins: any) => ins.id === instance.id),
                );

                let instnaces2 = initInstances.filter(
                    (instance: any) => !protectedInstances.some((ins: any) => ins.id === instance.id),
                );
                setUnprotectedInstances(instnaces.concat(instnaces2));
            } else {
                setUnprotectedInstances(unprotectedInstances);
            }
        }
